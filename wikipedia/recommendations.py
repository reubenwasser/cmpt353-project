...
best= gpd.read_file('best_destinations_vancouver.geojson')
df2 = pd.read_csv('joined_data.csv')
...
def nearby_locations(pt, typep, place):
 for j in df2['coords']:
...
df['location_name'] = df.apply(lambda x: get_location(x.lat, x.lng), axis=1)
df['coords'] = df.apply(lambda x: get_coords(x.lat, x.lng), axis=1)
df2['coords'] = df2.apply(lambda x: get_coords(x.lat, x.lon), axis=1)
df.apply(lambda x: nearby_locations(x.coords, x.tourism, x.location_name), axis = 1)
